...
import type { PendingRegistration, Team, Tournament } from "@/types/tournament"
...
export default function AdminPendingRegistrations({
  ...
  const revertToPending = async (registrationId: number) => {
    await supabase
  ...
  const handleApproveRegistration = async (
    ...
    try {
      ...
      if (onRegistrationUpdated) await onRegistrationUpdated()
      setIsReviewDialogOpen(false)
      setReviewingRegistration(null)
    } catch (approveError) {
      console.error("Error approving registration:", approveError)
      setError(formatSupabaseError(approveError, "Failed to approve registration"))
    } finally {
  ...
  const handleRejectRegistration = async (
    ...
    try {
      ...
      console.error("Error rejecting registration:", rejectErr)
      setError(formatSupabaseError(rejectErr, "Failed to reject registration"))
    } finally {
      setIsSubmitting(false)
      setBusyId(null)
    }
  ...
  const quickApprove = async (registration: PendingRegistration) => {
    if (!confirm(`Approve registration for "${registration.teamName}"? They will be added as unpaid.`)) {
      return
    }
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-slate-600 outdoor-text">
            Review public signups. Approved teams are added unpaid ($40) until you mark them paid.
          </p>
        </div>
      </div>
      ...
      {sortedRegistrations.length === 0 ? (
        <Card className="tournament-card">
          <CardContent className="pt-6">
            <div className="text-center py-8">
              ...
              <p className="text-slate-600">No team registrations have been submitted yet.</p>
            </div>
          </CardContent>
        </Card>
      ) : (
        <div className="space-y-4">
          {sortedRegistrations.map((registration) => {
            const rowBusy = busyId === registration.id
            return (
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-slate-500">
                        Submitted {new Date(registration.submittedAt).toLocaleDateString()} at{" "}
                        {new Date(registration.submittedAt).toLocaleTimeString()}
                      </p>

                      {registration.reviewedAt && (
                        <p className="text-sm text-slate-500">
                          Reviewed by {registration.reviewedBy} on{" "}
          ...
          {reviewingRegistration && (
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <p className="text-slate-600">{reviewingRegistration.players.join(" & ")}</p>
                <p className="text-slate-600">
                  Tournament: {getTournamentName(reviewingRegistration.tournamentId)}
                </p>
                {reviewingRegistration.contactInfo && (
                  <p className="text-slate-600">Contact: {reviewingRegistration.contactInfo}</p>
                )}
                <p className="text-sm text-slate-500">
              ...
              <div>
                <Label htmlFor="admin-notes" className="outdoor-text">
                ...
                <Textarea
                  ...
                  rows={3}
                  maxLength={500}
                  disabled={reviewingRegistration.status !== "pending"}
                />
              </div>
              ...
              {reviewingRegistration.status === "pending" ? (
                <div className="flex gap-3 pt-4">
                  <Button
                    onClick={() =>
                      handleRejectRegistration(reviewingRegistration, adminNotes)
                    }
                    disabled={isSubmitting}
                    variant="outline"
                  ...
                  <Button
                    onClick={() =>
                      handleApproveRegistration(reviewingRegistration, adminNotes)
                    }
                    disabled={isSubmitting}
